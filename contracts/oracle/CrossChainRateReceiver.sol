...
import { ILayerZeroReceiver } from "@layerzerolabs/lz-evm-v1-0.7/contracts/interfaces/ILayerZeroReceiver.sol";
...
abstract contract CrossChainRateReceiver is ILayerZeroReceiver, Ownable {
    error SenderMustBeLayerZeroEndpoint();
    error SourceChainIdDoesNotMatch(uint16 srcChainId, uint16 expectedSrcChainId);
    error SourceOAppNotExpected(address srcAppId, address expectedSrcAppId);
    ...
    function lzReceive(uint16 _srcChainId, bytes memory _srcAddress, uint64, bytes calldata _payload) external {
        if (msg.sender != layerZeroEndpoint) {
            revert SenderMustBeLayerZeroEndpoint();
        }
        ...
        if (_srcChainId != srcChainId) {
            revert SourceChainIdDoesNotMatch(_srcChainId, srcChainId);
        }
        if (srcAddress != rateProvider) {
            revert SourceOAppNotExpected(srcAddress, rateProvider);
        }
