...
contract DVNFeeLibMock is Ownable, IDVNFeeLib {
        ...
        returns (uint256)
    {
        ...
        //        uint256 priceFeedFee = ILayerZeroPriceFeed(_params.priceFeed).getFee(_params.dstEid, callDataSize,
        // _dstConfig.gas);
        ...
        (uint256 fee,,, uint128 nativePriceUSD) =
            ILayerZeroPriceFeed(_params.priceFeed).estimateFeeOnSend(_params.dstEid, callDataSize, _dstConfig.gas);

        return _applyPremium(
            fee, _dstConfig.multiplierBps, _params.defaultMultiplierBps, _dstConfig.floorMarginUSD, nativePriceUSD
        );
    }
        ...
        returns (uint256)
    {
        ...
        uint256 callDataSize = _getCallDataSize(_params.quorum);
        (uint256 fee,,, uint128 nativePriceUSD) =
            ILayerZeroPriceFeed(_params.priceFeed).estimateFeeByEid(_params.dstEid, callDataSize, _dstConfig.gas);
        return _applyPremium(
            fee, _dstConfig.multiplierBps, _params.defaultMultiplierBps, _dstConfig.floorMarginUSD, nativePriceUSD
        );
    }
    ...
    function _decodeDVNOptions(bytes calldata _options) internal pure returns (uint256) {
        ...
        if (cursor != _options.length) {
            revert DVNOptions.DVN_InvalidDVNOptions(cursor);
        }
